...
apps = [
    ('Architecture QA', 'Ask Questions'),
    ('Impact Analysis', 'Trace Effects'),
    ('Risk Assessment', 'Evaluate Risk'),
    ('Data Lineage', 'Track Data Flow'),
    ('Refactor Suggest', 'Improve Code'),
]
...
# Agents row 1
a1 = [
    ('Query Parser', 'Parse Intent', C['cyan']),
    ('Graph Retrieval', 'Search Graph', C['cyan']),
    ('Impact Analysis', 'Propagate Impact', C['cyan']),
    ('Risk Scoring', 'Score Risk', C['cyan']),
]
...
# Agents row 2
a2 = [
    ('Graph Builder', 'Build CPG (Offline)', C['green']),
    ('Lineage Builder', 'Build Lineage (Offline)', C['green']),
    ('LLM Reasoning', 'Explain (Optional)', C['purple']),
    ('Budget Controller', 'Control Cost', C['orange']),
]
...
kitems = [
    ('Code Property Graph', 'CPG Structure'),
    ('Data Lineage Graph', 'Data Flow'),
    ('Runtime Trace Graph', 'Execution Trace'),
    ('Business Ontology', 'Domain Model'),
    ('Embedding Index', 'FAISS Vector Index'),
    ('Neo4j Graph Store', 'Graph Database'),
]
...
ditems = [
    ('SQL AST Parser', 'Parse SQL'),
    ('CPG Builder', 'Multi-language'),
    ('Program Slicer', 'Slice Code'),
    ('Git Analyzer', 'Analyze Changes'),
    ('Call Graph', 'Extract Calls'),
    ('Column Lineage', 'Track Columns'),
]
dsx = 26.0
dsy = L4y + 0.8
...
# Data flow pipeline
steps = [
    ('User Query', 'Input', C['purple']),
    ('Parse Intent', 'Understand', C['blue']),
    ('Graph Retrieval', 'Search', C['cyan']),
    ('Impact Analysis', 'Propagate', C['amber']),
    ('Risk Scoring', 'Evaluate', C['red']),
    ('LLM Explain', 'Explain', C['purple']),
    ('Response', 'Output', C['green']),
]
...
# Principles
prins = [
    ('Code as Graph', 'CPG + Lineage + Call Graph\nGraph-based Analysis', C['blue']),
    ('Neuro-Symbolic', 'Deterministic + LLM\nHybrid Architecture', C['purple']),
    ('Token Budget', 'Predictable Cost\nAuto Degradation', C['orange']),
    ('Harness 3-Layer', 'Parse → Retrieve → Reason\nLayered Control', C['green']),
]
...
plt.savefig('/Users/miya/Documents/trae_projects/Harness-DB-CodeMind/architecture_en.png',
            dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor(),
            edgecolor='none')
plt.close()
print("Done! Saved to architecture_en.png")
